...
def main():
  ...
  while True:
    if not LCDQueue.empty():
      ...
      print(time_now)
      lcd_string(str(time_now),LCD_LINE_1)
      lcd_string(item,LCD_LINE_2)
      time.sleep(4)
      lcd_clear()
      lcd_light_off()
    if (global_Flag.getFlag() == True):
      # setメソッドが呼び出されるまでスレッドを待機させる
      LCDThState.wait_flag = True
      LCDThState.event.wait()
      LCDThState.wait_flag = False
